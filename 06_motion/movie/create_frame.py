...
if __name__ == "__main__":
	...
	for i in range(0, dim[2]):
		...
		ax[i,0].set_ylabel(str(int(bins[i])),fontsize=FS-5, labelpad=-4, fontweight="bold")
		...
		if (0 == i):
			ax[i,0].set_title(r'RAGA $\psi_{13}^1$', color=TCOLOR, fontsize=FS-2, fontweight="bold")
			ax[i,1].set_title(r'GR $\psi^1$', color=TCOLOR, fontsize=FS-2, fontweight="bold")

	ax[0,0].text(-0.3*dim[0], dim[2]//2 * dim[1], "Spokes / Frame", horizontalalignment='center', verticalalignment='center', color=TCOLOR, rotation="vertical", fontsize=FS-5, fontweight="bold")
