...
if __name__ == "__main__":
	...
	for i in range(0, 5):
		...
		if (0 == i):
			...
			ax[0,i].text(0.99*dim[0] + 5//2 * dim[0], -0.2*dim[1], "Spokes after Inversion", horizontalalignment='center', verticalalignment='bottom', color=TCOLOR, rotation="horizontal", fontsize=FS, fontweight="bold")

			ax[0,i].set_ylabel(r"RAGA", fontsize=FS)
			ax[1,i].set_ylabel(r"GA", fontsize=FS)
			ax[2,i].set_ylabel(r"$|$RAGA - GA$|$", fontsize=FS)
