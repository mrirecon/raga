...
if __name__ == "__main__":
	...
	fig, ax = plt.subplots(4, len(traj), dpi=300, figsize=(7, 5)) #figsize=(3, 3)
	...
	for x in range(0, 4):
		for j in range(0, len(traj)):
			...
			for i in range(0, np.shape(traj[j])[2], steps):
				...
				if (1 == j):
					ax[x,j].text(1.15*traj[j][0,-1,i,ind], 1.15*traj[j][1,-1,i,ind], str(i), fontsize=FS-12, horizontalalignment='center', verticalalignment='center', color=spoke.get_facecolors()[0])
			...
			if ((0 == x) and (0 == j)):
				ax[x,j].text(np.shape(traj[j])[1]/3, -1.1*np.shape(traj[j])[1]/2, \
					...
					fontsize=FS-14, horizontalalignment='center', verticalalignment='center', \
			...
			if (3 == x):
				ax[x,j].text(0, -1.9*np.shape(traj[j])[1]/2, str(int(spokes[j])), fontsize=FS-7, horizontalalignment='center', verticalalignment='center', rotation='horizontal')

			if (0 == j):
				ax[x,j].text(-1.5*np.shape(traj[j])[1]/2, 0, choice[x][3], fontsize=FS-5, horizontalalignment='center', verticalalignment='center', rotation='vertical')

	ax[1,0].text(-2.9*np.shape(traj[j])[1]/2, 0, "RAGA\nApproximations", fontsize=FS-5, horizontalalignment='center', verticalalignment='center', rotation='vertical')

	ax[3,2].text(-0.5*np.shape(traj[j])[1], -2.5*np.shape(traj[j])[1]/2, "Spokes", fontsize=FS-5, horizontalalignment='center', verticalalignment='center', rotation='horizontal')

	ax[0,0].text(-3.3*np.shape(traj[j])[1]/2, 1.4*np.shape(traj[j])[1]/2, "B", fontsize=FS+5, horizontalalignment='center', verticalalignment='center', rotation='horizontal', weight='bold')

	fig.subplots_adjust(right=0.8)
	cbar_ax = fig.add_axes([0.85, 0.08, 0.05, 0.7])
	...
	cbar.ax.set_title('Time\nIndices',fontsize=FS-10)
	cbar.ax.tick_params(axis='y', labelsize=FS-10)
